...
| `AUTO_CURATE_*` | optional Render Env | nein | Schwellen für `/api/admin/auto-curate`: `AI_MIN` (4), `CORPUS_MIN` (0.55), `AI_REJECT` (2), `CORPUS_REJECT` (0.30), `WERK_MIN` (0.55), `CONCEPT_MIN` (0.65 — seit 2026-06-22 von 0.68 gesenkt: bei 0.68 saßen 29 sonst freigabefähige Einträge in der Totzone [0.62,0.68), cn-Median im Pool ist 0.703), `CONCEPT_REJECT` (0.62). Niedriger = aggressivere Auto-Freigabe. Default konservativ. `CONCEPT_*` = dritter (Begriffs-)Anker des triangulierten Schutzwalls. |
...
| `QUESTIONS_ANSWER_SIM` | optional (Build/CI Env) | nein | Cosine-Schwelle (Default 0.72), ab der ein *späterer* Eintrag als „Antwort" auf die Schlussfrage eines früheren gilt (`build-resonanzen-index.ts:writeQuestions` → `resonanzen-questions.json`, Fragenansicht `/fragen`, Erkenntnisse-Phase 1). Höher = strenger (weniger „beantwortet"). |
| `ERKENNTNIS_CAND_DISTINCT_MIN` | optional (Build/CI Env) | nein | Distinktheits-Schwelle (Default 0.20; `1−maxCosine` der Antwort zum kuratierten Kanon) für **Erkenntnis-Kandidaten** (`build-resonanzen-index.ts:writeErkenntnisCandidates` → `resonanzen-erkenntnis-candidates.json`, Erkenntnisse-Phase 2). Antwort-zentrisch: eine Antwort, die offene Fragen löst + distinkt ist. Admin destilliert Kernsatz + bestätigt (`/api/admin/{distill,confirm}-erkenntnis` → `resonanzen-erkenntnisse.json`). Höher = selektiver. |
